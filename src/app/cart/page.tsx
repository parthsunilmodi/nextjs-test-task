...
import {addProductToCart, decreaseProduct, increaseProduct} from "../../redux/slice/product/productApi";
            ...
            {cart?.map((item: ICart, id) => (
              ...
              >
                ...
                <div className="flex items-center justify-evenly p-2 w-[50%] gap-3 border-1 rounded-lg h-14">
                  <FontAwesomeIcon icon={faMinus} className="bg-[#8b9dc3] p-3 rounded" onClick={() => handleDecrease(item)}/>
                    {item.amount}
                  <FontAwesomeIcon icon={faPlus} className="bg-[#8b9dc3] p-3 rounded" onClick={() => handleIncrement(item)}/>
                  <FontAwesomeIcon
                    icon={faMinusCircle}
                    color="#c53030"
                    size="2x"
